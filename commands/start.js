...
module.exports = {
	data: new SlashCommandBuilder().setName('start').setDescription('Start a new learning session.'),
	async execute(interaction) {
		...
		if (learningSessions.has(userId)) {
			await interaction.reply({
				content:
					'You already have an active learning session. Finish that one or use /stop.',
				ephemeral: true,
			});
			return;
		}
	...
	async onButton(interaction) {
		...
		if (interaction.customId === 'reveal') {
			...
			interaction.customId === 'remembered' ||
